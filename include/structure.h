...
namespace object
{
    ...
    struct ecs
    {
        ...
        struct system
        {
            ...
            std::vector<void (*)(ecs&, system&, void *)> functions; /** @brief Various user-defined or default functions that can be run on the `System`.*/
            ...
            {
                functions = std::vector<void (*)(object::ecs&, system&, void *)>(functionSize, [](object::ecs&, system&, void *){});
                initialized = false;
            }
            ...
            void createFunction()
            {
                functions.push_back([](ecs&, system&, void *){});
            }
            ...
            {
                functions[index](container, *this, data);
            }
        ...
        struct ComponentManager
        {
             ...
             */
            ComponentManager(entity numberOfEntities) : indexMaps(idCount, std::vector<size_t>(numberOfEntities))
            {
            ...
            void removeEntity(entity entity)
            {
                ...
                {
                    size_t index = indexMaps[id][entity];
                    if(index == (size_t)-1)
                        continue;
            ...
            {
                while(indexMaps.size() < idCount)
                {
                    indexMaps.push_back(std::vector<size_t>(numberOfEntities));
                    componentArrays.push_back(ComponentArray(spaceBuffer[idCount-1], complexBuffer[idCount-1]));
                }
            }

            bool complex(uint32_t id)
            {
                return componentArrays[id].complex();
            }
            ...
            {
                ComponentArray& array = componentArrays[id];
                size_t& index = indexMaps[id][e];
                return array.addComponent<T>(index, component);
            }
            ...
            T addComponent(entity e, uint32_t id, const T& component)
            {
                ComponentArray& array = componentArrays[id];
                size_t& index = indexMaps[id][e];              
                return array.addComponent<T>(indexMaps[id][e], component);
            }
            ...
            void share(entity e, entity share, uint32_t id)
            {
                ...
                indexMaps[id][e] = indexMaps[id][share];
            }
            ...
            T& getComponent(entity e)
            {
                ...
                return componentArrays[id].getComponent<T>(indexMaps[id][e]);
            }
            ...
            {
                uint32_t id = ComponentType<std::vector<T>>::id;
                size_t idx = indexMaps[id][e];
                ComponentArray& array = componentArrays[id];
            ...
            {
                uint32_t id = ComponentType<T>::id;
                return componentArrays[id].getDefaultComponent<T>();
            }
            ...
            void setComponent(entity e, const T& update)
            {
                uint32_t id = ComponentType<T>::id;
                size_t index = indexMaps[id][e];
                size_t offset = componentArrays[id].setComponent<T>(index, update);

                size_t size = indexMaps[id].size();
                for(int i=0; i<size; i++)
                {
                    if(indexMaps[id][i] > index)
                        indexMaps[id][i] += offset;
                }
            }
            ...
            private:
                void remove(uint32_t id, size_t index, entity e)
                {
                    ...
                    for(int i=0; i<size; i++)
                    {
                        if(indexMaps[id][i] > index && indexMaps[id][i] != (size_t)-1)
                        {
                            indexMaps[id][i] -= offset;
                        }
                    } 
        ...
        struct SystemManager
        {
            ...
            static std::vector<size_t> spaceBuffer;
            ...
            SystemManager(entity numberOfEntities) : stores(idCount, system(functionIndex)), supplements(idCount, SystemSupplement(numberOfEntities)) {}
            ...
            void runFunction(ecs& container, uint8_t index, void *data)
            {
                for(system& store : stores)
                {
                    ...
                    {
                        store.runFunction(container, index, data);
                    }
                }
            }
            ...
            system& createSystem(const T& instance, int32_t priority, uint32_t& id)
            {
                ...
                auto supplement = supplements[id];
                ...
                if(id > 0)
                {
                    do
                    {
                        ...
                        if(stores[total].priority > priority)
                        {
                ...
                system& system = stores[total];
                supplements[total] = supplement;
            ...
            private:
                ...
                {
                    stores[to] = stores[from];
                    supplements[to] = supplements[from];
                }
        ...
        struct EntityManager
        {
            ...
            entity createEntity()
            {
                ...
                {
                    componentBitmaps.push_back(std::vector<bool>(ComponentManager::idCount + 1));
                }
            ...
            void removeEntity(entity entity)
            {
                ...
                componentBitmaps[entity] = std::vector<bool>(ComponentManager::idCount + 1);
        ...
        T& getComponentAt(entity e, size_t index)
        {
            ...
            return componentManager.getComponentAt<T>(e, index);
        }

        template<typename T>
        size_t getCompressedIndex(entity e)
        {
            return componentManager.getCompressedIndex<T>(e);
        }
        ...
        private:
            void addComponentConfiguration(entity entity, uint32_t id)
            {
                ...
                for(uint32_t i=0; i<systemManager.idCount; i++)
                {
                    if(systemManager.bitmapMatches(i, bitmap, id))
                    {
                        systemManager.insertEntity(*this, entity, i);
                    }
                }
            }
    };      
...
template<> // DONE
struct Serialization<object::ecs::EntityManager>
{
...
template<> // DONE
struct Serialization<object::ecs::ComponentArray>
{
    static size_t length(const object::ecs::ComponentArray& data)
    {
        return 
    ...
    static size_t serialize(const object::ecs::ComponentArray& value, std::vector<uint8_t>& stream, size_t index)
    {
        size_t count = 0;
    ...
    static object::ecs::ComponentArray deserialize(std::vector<uint8_t>& stream, size_t index)
    {
        object::ecs::ComponentArray result = object::ecs::ComponentArray();
        size_t count = 0;
...
template<> // static vars
struct Serialization<object::ecs::ComponentManager>
{
    static size_t length(const object::ecs::ComponentManager& data)
    {
        return 
    ...
    static size_t serialize(const object::ecs::ComponentManager& value, std::vector<uint8_t>& stream, size_t index)
    {
        size_t count = 0;

        count += object::serialize(value.idCount, stream, index + count);
    ...
    static object::ecs::ComponentManager deserialize(std::vector<uint8_t>& stream, size_t index)
    {
        object::ecs::ComponentManager result = object::ecs::ComponentManager();
        size_t count = 0;
        ...
        result.componentArrays = object::deserialize<std::vector<object::ecs::ComponentArray>>(stream, index + count);
...
template<> // pointer var and function pointer var
struct Serialization<object::ecs::system>
{
    static size_t length(const object::ecs::system& data)
    {
        return 
            object::length(data.priority) + 
            object::length(*data.index) +
            object::length(data.initialized) +
            object::length(data.instance);
    }

    static size_t serialize(const object::ecs::system& value, std::vector<uint8_t>& stream, size_t index)
    {
        size_t count = 0;

        count += object::serialize(value.priority, stream, index + count);
        count += object::serialize(*value.index, stream, index + count);
    ...
    static object::ecs::system deserialize(std::vector<uint8_t>& stream, size_t index)
    {
        object::ecs::system result = object::ecs::system();
        size_t count = 0;
        ...
        {
            *result.index = object::deserialize<uint32_t>(stream, index + count);
        }
...
template<> // DONE
struct Serialization<object::ecs::SystemSupplement>
{
    static size_t length(const object::ecs::SystemSupplement& data)
    {
        return
    ...
    static size_t serialize(const object::ecs::SystemSupplement& value, std::vector<uint8_t>& stream, size_t index)
    {
        size_t count = 0;
    ...
    static object::ecs::SystemSupplement deserialize(std::vector<uint8_t>& stream, size_t index)
    {
        object::ecs::SystemSupplement result = object::ecs::SystemSupplement();
        size_t count = 0;
...
template<> // static vars
struct Serialization<object::ecs::SystemManager>
{
    static size_t length(const object::ecs::SystemManager& data)
    {
        return
            object::length(data.idCount) +
    ...
    static size_t serialize(const object::ecs::SystemManager& value, std::vector<uint8_t>& stream, size_t index)
    {
        size_t count = 0;
    ...
    static object::ecs::SystemManager deserialize(std::vector<uint8_t>& stream, size_t index)
    {
        object::ecs::SystemManager result = object::ecs::SystemManager();
        size_t count = 0;
        ...
        result.stores = object::deserialize<std::vector<object::ecs::system>>(stream, index + count);
        count += object::length(result.stores);

        result.supplements = object::deserialize<std::vector<object::ecs::SystemSupplement>>(stream, index + count);
...
template<> // DONE
struct Serialization<object::ecs>
{
